...
如果这个 Spatial 对象是 Geometry 类型，那么问题就简单了。因为每个 Geometry 内部只有一个Mesh，所以它会直接调用 `Mesh#getBound()` 来获得包围体。同理，你还可以通过调用 `Geometry#setModelBound()` 和 `Geometry#updateModelBound()` 方法来间接调用 `Mesh#setBound()` 和 `Mesh#updateBound()`。
